...
st.set_page_config(page_title="App 1 - Dashboard", page_icon="📊")

def main_app():
    st.title("📊 App 1 - Dashboard")
    st.write(f"Welcome, {st.session_state.user_email}!")
    
    st.header("Dashboard Content")
    st.info("This is App 1 - You can add your specific functionality here")
    ...
    with col3:
        st.metric("Orders", "856", "-2%")
    
    # Quick access to App 2 with shared session
    st.header("Quick Access")
    app2_url = get_cross_app_url(8502, st.session_state.access_token)
    st.markdown(f'<a href="{app2_url}" target="_blank">🔗 Open App 2 (Shared Session)</a>', unsafe_allow_html=True)
